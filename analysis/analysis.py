...
def ions(universe):
    ...
    print('To select the ions in the bulk solution, use "resname (atom) and (prop z <', min_clay[2], 'or prop z >', max_clay[2], ')" for the atom selection')
...
def position_density(u, atoms, dyn):
    ...
    # Create dataframes
    x_pos_dens=pd.DataFrame(atom_density.results.x.pos, columns=['pos'])
    y_pos_dens=pd.DataFrame(atom_density.results.y.pos, columns=['pos'])
    z_pos_dens=pd.DataFrame(atom_density.results.z.pos, columns=['pos'])
    
    # Create coordinate column info
    x_index=pd.Index(np.linspace(0, 36.4, atom_density.nbins))
    y_index=pd.Index(np.linspace(0, 45.1, atom_density.nbins))
    ...
    x_pos_dens['std of position density']=atom_density.results.x.pos_std
    y_pos_dens['std of position density']=atom_density.results.y.pos_std
...
def atom_contacts(u, atom_group1, atom_group2, contact_rad):
    ...
    at_con = np.array(timeseries)
    
    at_con_df = pd.DataFrame(at_con, columns=['Frame',
                                  '# Contacts'])
    
    timestep=float(input('What is the time between frames? Give answer in nanoseconds: '))
    at_con_df['Time (ns)']=(at_con_df['Frame'])* timestep
